...
export const goalController = {
	...
	show: async (month?: string) => {
		if (!month) throw new Error('Nenhum mês informado!')
		...
		await Promise.all(
			goal.companies.map(async company => {
				const rawCompany: CompanyRaw = await db
					.table('companies')
					.get(company.id)
				...
				let companyGoal = 0
				let companySold = 0
				const companySellers: GoalSeller[] = []

				await Promise.all(
					company.sellers.map(async seller => {
						const rawSeller: SellerRaw = await db
							.table('sellers')
							.get(seller.id)
						...
						const requests: RequestRaw[] = (
							await db.table('requests').toArray()
						).filter(
							({representada, vendedor}: RequestRaw) =>
				...
				const eCommerceRequests: RequestRaw[] = (
					await db.table('requests').toArray()
				).filter(
					({representada, vendedor}: RequestRaw) =>
