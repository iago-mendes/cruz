...
const EditRequestProductModal: React.FC<EditRequestProductModalProps> = ({
	...
	useEffect(() => {
		if (selected.companyId !== '' && selected.product.id !== '')
			companyController
				...
				.then(company => {
					const tmpRawProduct = company.produtos.find(
						({_id}) => _id === selected.product.id
					)
					setRawProduct(tmpRawProduct ? tmpRawProduct : defaultProduct)

					const clientCompanyTable = tmpRawProduct.tabelas.find(
						table => table.id === selected.clientCompanyTableId
					)
					...
					setProductTable({
						id: tmpProductTable.id,
						price: tmpProductTable.preco
					})
				})
	...
	useEffect(() => {
		if (productTable.price !== 0) updatePrice(productTable.price)
	}, [productTable, discounts])

	useEffect(() => {
		if (selected.product.id !== '') {
			const existingIndex = products.findIndex(
				({id}) => id === selected.product.id
			)
			...
			if (existingIndex >= 0) {
				...
				else tmpProducts[existingIndex] = selected.product
		...
		<ModalContainer isOpen={isOpen} setIsOpen={setIsOpen}>
			...
			<Container className="with-fixed-header">
				<div className="info">
					...
					<div className="texts">
						<h3>{rawProduct.nome}</h3>
						<span>{rawProduct.codigo}</span>
					</div>
				</div>
				...
				<div className="summary">
					<span>
						Valor subtotal (sem impostos):{' '}
						<strong>{formatPrice(calcSubtotal(), true)}</strong>
					</span>
				</div>
			</Container>
		</ModalContainer>
