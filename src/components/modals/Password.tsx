...
const PasswordModal: React.FC<PasswordModalProps> = ({
	...
	function handleSubmit() {
		...
		api
			.put(`change-password/${role}/${id}`, data)
			.then(() => {
				setIsOpen(false)
				sucessAlert('Senha enviada com sucesso!')

				if (handleSendCredentialsViaMail) handleSendCredentialsViaMail(newPwd)
			})
			.catch(error => {
		...
		<ModalContainer isOpen={isOpen} setIsOpen={setIsOpen}>
			<Container>
				<div className="options">
					...
					<button title="Gerar senha aleatória" onClick={createRandomPwd}>
						<FaRandom />
