...
const CompanyForm: React.FC<CompanyFormProps> = ({
	...
	useEffect(() => {
		if (company) {
			...
			if (company.nome_fantasia) setNomeFantasia(company.nome_fantasia)
			if (company.cnpj) setCnpj(company.cnpj)
			if (company.telefones) setTelefones(company.telefones)
			if (company.email) setEmail(company.email)
			if (company.comissao) setComissao(company.comissao)
			if (company.descricao_curta) setDescricaoCurta(company.descricao_curta)
			if (company.descricao) setDescricao(company.descricao)
			if (company.site) setSite(company.site)
			if (company.tabelas) setTabelas(company.tabelas)
			if (company.condicoes) setCondicoes(company.condicoes)
		}
	}, [company])
	...
	function validateFields() {
		...
		if (comissao.porcentagem === 0)
			return {
				areFieldsValid: false,
				warning: 'Você precisa informar a porcentagem de comissão.'
			}

		if (tabelas.length === 0)
			return {
				areFieldsValid: false,
				warning: 'Você precisa informar pelo menos uma tabela.'
			}

		if (condicoes.length === 0)
			return {
				areFieldsValid: false,
				warning: 'Você precisa informar pelo menos uma condição de pagamento.'
			}
					...
					{comissao.obs.map((obs, index) => (
						<li key={index} className="obs">
							<input
								...
								value={obs}
								onChange={e =>
									handleComissaoChange(e.target.value, 'comissao_obs', index)
								}
							/>
							<button
								type="button"
								onClick={() => handleRemoveComissaoObs(index)}
							>
								<FiMinus />
					...
					{condicoes.map((condicao, index) => (
						<li key={index}>
							<input
								...
								value={condicao.nome}
								onChange={e =>
									handleChangeCondition(e.target.value, index, 'name')
								}
