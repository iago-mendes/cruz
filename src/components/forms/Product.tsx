...
const ProductForm: React.FC<ProductFormProps> = ({
	...
	useEffect(() => {
		api
			.get(`companies/${companyId}/raw`)
			.then(({data}: {data: {tabelas: Table[]}}) => {
	...
	useEffect(() => {
		if (product) {
			if (product.nome) setNome(product.nome)
			if (product.codigo) setCodigo(product.codigo)
			if (product.unidade) setUnidade(product.unidade)
			if (product.peso) setPeso(product.peso)
			if (product.volume) setVolume(product.volume)
			if (product.ipi) setIpi(product.ipi)
			if (product.st) setSt(product.st)
			if (product.comissao) setComissao(product.comissao)
		}
	}, [product])
			...
			if (product && product.tabelas) {
				if (product.tabelas.length === companyTables.length)
					...
					const tmpTables: ProductTable[] = companyTables.map(companyTable => {
						const productTable = product.tabelas.find(
							({id}) => id === companyTable._id
						)

						if (productTable) return productTable
						else return {id: companyTable._id, preco: 0}
					})
	...
	function validateFields() {
		...
		if (codigo === '')
			return {
				...
				warning: 'Você precisa informar o código.'
			}

		if (unidade === '')
			return {
				areFieldsValid: false,
				warning: 'Você precisa informar a unidade.'
			}
	...
	function handleSubmit() {
		...
		if (method === 'post') {
			api
				.post(`companies/${companyId}/products`, data)
				.then(() => {
